    ...
    render: function(){
      ...
      return (
        React.DOM.a({className: "Product clearfix", href: "#", onClick: this.onClick, ref: "productContent", id: domid}, 
          ...
          React.DOM.button({className: "btn btn-default btn-sm pull-right", type: "submit"}, "添加"), 
  ...
  var ProductList = React.createClass({displayName: 'ProductList',
    render: function(){
      ...
      var productNodes = this.props.data.map(function(product){
        return (
          Product({name: product.name, price: product.price, productId: product.id, productClick: obj.props.productClick, orderSubmit: this.props.orderSubmit})
        );
      ...
      if(this.props.show == 'show'){
        ...
        return (
          ...
          React.DOM.div({className: "PreOrder clearfix", style: this.props.style}, 
            ...
            React.DOM.button({type: "button", onClick: this.onClick, className: "btn btn-mwm pull-right"}, "确定"), 
            React.DOM.button({type: "button", onClick: this.cancelOrder, className: "btn btn-default pull-right mr-20"}, "取消")
          )
        );
      }else{
        return (
          ...
          React.DOM.div({className: "col-md-8"}, 
            React.DOM.div({className: "white-container"}, 
              React.DOM.div({className: "mb-15"}, 
                ...
                React.DOM.small({className: "text-muted"}, "此界面由店员操作")
              ), 
              React.DOM.div(null
