  ...
  var Product = React.createClass({displayName: 'Product',
    onClick: function(e){
      alert(this.refs.productContent.getDOMNode().value);
      var preOrderLeft = $('#'+this.refs.productContent.getDOMNode().id).offset().left;
      var preOrderTop = $('#'+this.refs.productContent.getDOMNode().id).offset().top - 2;
      var preOrderWidth = $('#'+this.refs.productContent.getDOMNode().id).outerWidth();
    ...
    render: function(){
      ...
      return (
        React.DOM.a({className: "Product clearfix", href: "#", ref: "prudoctContent", value: domid}, 
          React.DOM.span({className: "pull-left"}, 
            this.props.name
          ), 

          React.DOM.span({className: "pull-right"}, 
            React.DOM.button({className: "btn btn-default btn-sm", onClick: this.onClick, type: "submit"}, "添加")
          ), 
          React.DOM.span({className: "pull-right"}, 
            "¥", this.props.price
          )
